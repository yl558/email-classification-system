#!C:\\Users\\yl558\\AppData\\Local\\Programs\\Python\\Python37\\python.exe
...
def main():
    ...
    word_index_file = os.path.join('data','word_index.txt')
    word_index = load(word_index_file)
    
    if len(sys.argv) > 1:
        text_file_name = sys.argv[1]
        #text_file = os.path.join('uploads', text_file_name)
        text_file = text_file_name
    else:
        text_file = os.path.join('uploads','38758')
    text = read(text_file)
    ...
    best_model_file = os.path.join('data','bestmodel')
    ...
    label_index_file = os.path.join('data','label_index.txt')
